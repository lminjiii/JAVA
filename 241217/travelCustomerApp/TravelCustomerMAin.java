...
public class TravelCustomerMAin {
    public static void main(String[] args) {
        ...
        //1. 고객명단을 추가된 순서대로 정렬하기
        System.out.println("1. 고객명단을 추가된 순서대로 정렬하기");
        customers.stream()
        ...
        System.out.println("4. 21세 이상 고객명단의 이름만 오름차순으로 출력하기");
        customers.stream().filter(x -> x.getAge() >= 21)
                .map(x -> x.getName())
                .sorted()
                .forEach(x -> System.out.println(x));
        //5. 21세 이상 고객명단 전체를 내림차순으로 출력하기
        System.out.println("5. 21세 이상 고객명단 전체를 내림차순으로 출력하기");
        customers.stream()
