...
public class AdminService {
    ...
    public boolean deleteItem(String itemName) {
        ...
        //2. 존재하면 삭제 요정 후 true 리턴 그렇지 않으면 false 리턴
        if(index != -1){
            //삭제 요청
    ...
    public boolean updateStock(String itemName, int updateStock) {
        ...
        if(index != -1){
            //수정할 제품 찾기
            Product findProuct = repository.findByName(itemName);
            //재고 수정하기
            int updateValue = findProuct.getStock() + updateStock;
            findProuct.setStock(updateValue);
            //수정요청
            repository.update(index, findProuct);
            return true;
        }else {
            //수정 실패
            return false;
