    ...
    private static void admin() {
        ...
        while (true) {
            ...
            if (twoNum == 1) {
                ...
                System.out.println(product[newNum] + "를(을) 무엇으로 바꾸시겟습니까?");
                String twoProduct = scanner.next();
                product[newNum] = twoProduct;
                System.out.println(product[newNum] + "의 가격은 얼마 입니까?");
                int twoPrice = scanner.nextInt();
                price[newNum] = twoPrice;
                System.out.println(product[newNum] + "의 재고를 몇 개 등록 하시겠습니까?");
                int twoStock = scanner.nextInt();
                stock[newNum] = twoStock;
                System.out.println("메뉴 변경이 완료되었습니다!");
            } else if (twoNum == 2) {
                System.out.println("가격을 변경하실 메뉴의 번호를 입력하세요 ( 1 ~ 3 )");
                int priceNum = scanner.nextInt();
                System.out.println(product[priceNum] + "의 가격을 얼마로 바꾸시겠습니까?");
                int newPrice = scanner.nextInt();
                price[priceNum] = newPrice;
                System.out.println("가격 변경이 완료 되었습니다!");
            } else if (twoNum == 3) {
                System.out.println("재고를 추가하실 메뉴의 번호를 입력하세요. ( 1 ~ 3 )");
                int stockNum = scanner.nextInt();
                System.out.println(product[stockNum] + "를(을) 몇 개 추가 하시겠습니까?");
                int newStock = scanner.nextInt();
                stock[stockNum] = newStock;
